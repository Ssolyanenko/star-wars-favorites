...
const MainScreen = ({navigation}:any) => {
  ...
  const handleAddToFavorites = (character) => {
    const index = favoriteCharacters.findIndex((fav) => fav.name === character.name);
    ...
    if (index === -1) {
      ...
      const updatedFavoriteCharacters = favoriteCharacters.filter((fav) => fav.name !== character.name);
      setFavoriteCharacters(updatedFavoriteCharacters);
      if (character.gender === 'male') {
        setMaleCount(maleCount - 1);
      } else if (character.gender === 'female') {
  ...
  const renderCharacter = ({ item }:any) => (
      ...
      <View style={styles.characterContainer}>
        <Text style={{ fontWeight: 'bold', color: '#000' }}>Name: {item.name}</Text>
        <Text style={{ fontWeight: 'bold', color: '#000' }}>Birth Year: {item.birth_year}</Text>
        <Text style={{ fontWeight: 'bold', color: '#000' }}>Gender: {item.gender}</Text>
        <Text style={{ fontWeight: 'bold', color: '#000' }}>Home World: {item.homeworld}</Text>
        <Text style={{ fontWeight: 'bold', color: '#000' }}>Species: {getSpeciesName(item.species[0])}</Text>
        <TouchableOpacity onPress={() => handleAddToFavorites(item)}>
        <Text  style={{ color: isFavorited(item) ? 'red' : 'green', fontWeight: 'bold', textAlign:'center', marginVertical:10}}>Add to favorite</Text>
        </TouchableOpacity>
      </View>
      </TouchableOpacity>
  );
